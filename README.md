...
The current Raycast app opens by default to All Tasks, which groups active tasks under Inbox and their projects and sorts each group by due date. It also provides persistent Today, Upcoming, Inbox, project, Completed, and Trash views backed by a production SQLite database. Users can create, edit, move, complete, reopen, trash, and restore tasks with notes, priority, and optional due values. Projects can be created, renamed, and removed without deleting their tasks. Tasks remain in Inbox or one Project and the shared model also preserves zero or more global Labels. Quick Add and the full task form share a flat Project selector and optional Due Date presets for Today, Tomorrow, End of This Week, or a custom date. Quick Add also captures optional notes and defaults to Inbox with no due date. An actionable menu-bar command shows overdue tasks and tasks due through the end of the week, grouped into Today, Tomorrow, and Later This Week, with compact project names. Each task can be completed, opened, edited, or moved to recoverable Trash directly from its submenu, and the menu also opens the full creation form or the All Tasks view. It can be hidden from its own menu and restored by running `Worktodo Menu Bar` from Raycast. A `Backup & Restore` command exports complete version 2 JSON backups and validates, previews, and atomically restores version 1 or version 2 backups after explicit confirmation while retaining an automatic version 2 recovery backup.

The local MCP server exposes bounded tools to list projects and tasks, get one task, and create, update, move, complete, reopen, trash, or restore tasks. It never exposes arbitrary SQL or permanent deletion.
