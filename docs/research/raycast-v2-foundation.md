...
3. **Quick Add arguments are deliberately narrow.** A command accepts at most three root-search arguments, and their types are only text, password, and a manifest-defined dropdown. That is enough for a required title plus at most two shortcuts, but not for a dynamic project picker or rich due-date UI. A title-first no-view command and a separate full Form are compatible with the platform and with the official Todoist extension’s surface split. [current extension schema](https://www.raycast.com/schemas/extension.json), [arguments lifecycle](https://developers.raycast.com/information/lifecycle/arguments.md), [Todoist quick add](https://github.com/raycast/extensions/blob/main/extensions/todoist/src/quick-add-task.tsx), [Todoist create form](https://github.com/raycast/extensions/blob/main/extensions/todoist/src/create-task.tsx)
...
| Quick Add         | [schema](https://www.raycast.com/schemas/extension.json), [arguments](https://developers.raycast.com/information/lifecycle/arguments.md)                                                          | Confirmed                        | Required title plus no more than two optional root fields; dynamic metadata belongs in a Form.                        |
...
**Inference.** The reliable Quick Add contract is a required title followed by at most two optional shortcuts. There is no root-argument date picker, and a static dropdown cannot reflect projects created in the local database without rewriting the manifest. Therefore:

- title-only capture is first-class;
- an optional strict due string and/or another text shortcut may be possible;
- dynamic project and section selection belongs in a full Form;
- trying to encode every task field into root arguments would fight the platform.

**Unknown.** The product deliberately excludes natural-language parsing in v1. A compact, deterministic syntax for optional due/project metadata could still be useful, but its discoverability, accessibility, error handling, and interaction with Raycast argument focus need a host prototype rather than assumption.
...
4. **Quick Add stays title-first.** Root arguments are a speed path; the full Form is the metadata path. The platform’s three-argument cap should be treated as a product constraint, not worked around with brittle encoding.
...
- Whether a strict Quick Add shortcut syntax is worth its interaction and error cost.
...
### 6. Quick Add interaction limits — high

Prototype only the root argument surfaces: required title, optional text shortcut, static dropdown, alias-plus-space focus, validation failure, and a handoff to a full Form. Compare title-only speed with a deterministic due shortcut. The result should decide the argument contract without importing Todoist NLP. [arguments](https://developers.raycast.com/information/lifecycle/arguments.md), [aliases and hotkeys](https://manual.raycast.com/command-aliases-and-hotkeys.md)
